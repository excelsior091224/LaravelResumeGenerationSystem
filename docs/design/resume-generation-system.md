...
初期実装では複数のAI APIを契約せず、Google Gemini APIのみを採用する。中国、ロシア等の権威主義国家に拠点を置くAIサービスは採用対象外とする。現在のAPI応答では`gemini-2.5-flash-lite`が新規ユーザー向けに利用できないため、初期モデルは`gemini-3.5-flash-lite`を使用する。品質評価で不足が確認された場合は、同じGoogle API契約内で利用可能な上位モデルへ切り替える。これにより、API契約を増やさずに費用と品質を調整できる。
...
公式料金を基準にした概算はモデル世代により異なる。1回あたり入力4,000トークン、出力500トークンを前提に、契約時点の`gemini-3.5-flash-lite`の料金表から1,000回分を再計算する。料金、為替、モデル名は変更される可能性があるため、実装・契約時には公式料金表を再確認する。
